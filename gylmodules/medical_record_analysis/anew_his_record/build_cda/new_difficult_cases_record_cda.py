...
def assembling_header(admission_record: str, data: dict):
    ...
    admission_record = admission_record + xml_header.xml_header_file_info \
        ...
        .replace('{文档生成时间}', str(data.get('文档创建时间', '/')))
    ...
    admission_record = admission_record + xml_header.xml_header_authenticator2 \
        .replace('{签名时间}', str(data.get('签名时间', '/'))) \
        .replace('{医师id}', data.get("收治医师", '/') if data.get('收治医师') else '/') \
    ...
    admission_record = admission_record + xml_header.xml_header_authenticator2 \
        .replace('{签名时间}', str(data.get('签名时间', '/'))) \
        .replace('{医师id}', '/') \
        .replace('{医师名字}', data.get("主持医师", '/') if data.get('主持医师') else '/') \
        .replace('{显示医师名字}', '主持人')

    admission_record = admission_record + xml_header.xml_header_associated_person \
        .replace('{type_code}', 'RPCP') \
        .replace('{class_code}', 'ASSIGNED') \
        .replace('{name1}', '/') \
        .replace('{name2}', '/') \
        .replace('{name3}', '/')
    ...
    admission_record = admission_record + xml_header.xml_header_associated_time \
        .replace('{讨论时间}', str(data.get('讨论时间', '/'))) \
        .replace('{讨论地点}', data.get('讨论地点', '/'))
...
def assembling_body(admission_record: str, data: dict):
    taolunyijian = data.get('讨论意见')
    if not taolunyijian:
        taolunyijian = '/'
    elif type(taolunyijian) == dict:
        taolunyijian = taolunyijian.get('value', '/')
    ...
    admission_record = admission_record + xml_body.body_discharge_entry \
        ...
        .replace('{text}', '<text />') \
        .replace('{entry_observation_name}', '健康评估') \
        .replace('{entry1}', xml_body.body_section_entry
                 .replace('{entry_code}', 'DE06.00.018.00')
                 .replace('{entry_name}', '讨论意见')
                 .replace('{entry_body}', xml_body.value_st.replace('{value}', taolunyijian))) \
        .replace('{entry2}', xml_body.body_section_entry
                 .replace('{entry_code}', 'DE06.00.018.00')
                 .replace('{entry_name}', '主持人总结意见')
                 .replace('{entry_body}', xml_body.value_ts.replace('{value}', data.get('主持人总结意见', '/')))) \
        .replace('{entry3}', '') \
        .replace('{entry4}', '') \
        .replace('{entry5}', '') \
        .replace('{entry6}', '') \
