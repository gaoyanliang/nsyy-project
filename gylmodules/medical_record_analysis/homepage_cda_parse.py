...
def parse_cda_xml_document(root):
    ...
    parse_data['文档类型'] = root.find('hl7:code', namespaces).get('code')
    ...
    if patient_role:
        parse_data['住院号'] = patient_role.find('hl7:id', namespaces).get('extension') if patient_role.find('hl7:id',
                                                                                                             namespaces) is not None else '/'
        if patient_role.find('hl7:addr/hl7:houseNumber', namespaces) is not None:
        ...
        parse_data['患者性别'] = patient_role.find('hl7:patient/hl7:administrativeGenderCode', namespaces).get(
            'displayName') if patient_role.find('hl7:patient/hl7:administrativeGenderCode',
                                                namespaces) is not None else '/'
        parse_data['患者出生日期'] = patient_role.find('hl7:patient/hl7:birthTime', namespaces).get(
    ...
    if author:
        ...
        parse_data['文档创作者'] = author.find('hl7:author/hl7:assignedAuthor/hl7:assignedAuthor/hl7:name',
